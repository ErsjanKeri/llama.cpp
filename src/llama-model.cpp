...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // BSC: capture T2 — mmap done
    phases.t2_mmap_done = ggml_time_us();
    phases.f2_faults = llama_get_major_faults();
    ...
    if (params.pin_compute_weights) {
        int64_t t_mlock_start = ggml_time_us();
        size_t locked = 0;
        size_t attempted = 0;
        ...
        int64_t t_mlock_end = ggml_time_us();
        LLAMA_LOG_INFO("%s: pin_compute_weights: attempted %zu tensors, locked %.1f MiB, failed %zu tensors, took %.2f ms\n",
